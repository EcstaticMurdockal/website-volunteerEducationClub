...
#### 视频（已配置）
- [x] 2025支教纪录片 - B站链接：https://www.bilibili.com/video/BV1eGgtzLEuK/
- [x] 2024支教纪录片 - B站链接：https://www.bilibili.com/video/BV1RoWSenEi5/
...
- **视频功能**：
  - 视频托管在B站，点击后会在新标签页打开B站观看
  - 目前配置了2个视频（2025和2024支教纪录片）
  - 无需上传视频文件到项目中
- **PPT文件**：
...
- **宣传视频**：入口卡片，点击后显示2个视频列表（2025和2024），点击视频在新标签页打开B站观看
...
- **4个独立维度**：
  ...
  - 关注点：知识(K) vs 情感(E)
  - 行动方式：计划(P) vs 即兴(I)
- **32道精心设计的测试题**：每个维度8道题，确保测试精确性
- **科学的评分系统**：
  - 每道题只测试一个维度，维度之间相互独立
  - 根据4个维度的得分组合，计算出16种人格类型（2^4 = 16）
  - 显示每个维度的百分比倾向
- **详细的结果分析**：
  - 人格名称和emoji图标
  - 人格类型代码（如SGKP）
  - 维度组合说明（如：结构化 + 引导 + 知识 + 计划）
  - 核心特质描述
  - **维度倾向可视化**：用进度条展示4个维度的百分比倾向
  - 天赋优势分析（3-4条）
  - 改进建议（3-4条）
  - 实用建议（3条）
- **人格类型示例**：
  - 🏗️ 系统建筑师（SGKP）：结构化+引导+知识+计划
  - 🧭 知识探险家（SGKI）：结构化+引导+知识+即兴
  - 🌱 成长引路人（SGEP）：结构化+引导+情感+计划
  - ✨ 灵感点燃者（SGEI）：结构化+引导+情感+即兴
  - 🛡️ 温暖守护者（SAKP）：结构化+陪伴+知识+计划
  - 🤝 贴心伙伴（SAKI）：结构化+陪伴+知识+即兴
  - 🌸 心灵园丁（SAEP）：结构化+陪伴+情感+计划
  - ☀️ 阳光陪伴者（SAEI）：结构化+陪伴+情感+即兴
  - 🦉 智慧导师（FGKP）：灵活+引导+知识+计划
  - 🚀 创新先锋（FGKI）：灵活+引导+知识+即兴
  - 📚 人文导师（FGEP）：灵活+引导+情感+计划
  - 🎨 自由引导者（FGEI）：灵活+引导+情感+即兴
  - 🕊️ 温柔支持者（FAKP）：灵活+陪伴+知识+计划
  - 🍃 自然陪伴者（FAKI）：灵活+陪伴+知识+即兴
  - 💝 共情陪伴者（FAEP）：灵活+陪伴+情感+计划
  - 🦋 自由之友（FAEI）：灵活+陪伴+情感+即兴
- **测试流程**：
  1. 点击"开始测试"按钮
  2. 回答32道选择题（每题4个选项）
  3. 进度条实时显示完成度
  4. 自动计算并展示结果，包括维度倾向百分比
  5. 可重新测试
...
- 🎬 **视频功能优化**
  ...
  - 移除本地视频文件，减小项目体积
- 🖼️ **修复学校照片裁剪问题**
  - 大幅减小轮播高度：280px（桌面端）/ 220px（移动端）
  - 缩小Modal面板尺寸，整体更紧凑
  - 确保图片完整显示，不被裁剪

### v2.3.0 (2026-05-28)
- 🧪 **重大升级：人格测试系统**
  - 增加题目数量：从20题增加到32题，每个维度8题
  - 科学化维度设计：4个独立维度（教学风格、互动方式、关注点、行动方式）
  - 改进评分系统：每道题只测试一个维度，确保维度独立性
  - 新增维度百分比显示：直观展示在每个维度上的倾向程度
  - 优化人格描述：为每种人格添加维度组合说明
  - 更精确的测试结果：基于MBTI理论的科学测评
- 📸 **新增备课社课照片展示**
  - 为"每周支教备课社课"添加照片网格
  - 支持上传5张照片展示社课活动
